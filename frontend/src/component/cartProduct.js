...
const CartProduct = ({id,name,category,qty,total,price,image}) => {
  ...
  return (
    <div className='bg-slate-200 p-2 flex gap-4 rounded border-1 border-slate-300'>
        ...
        <div className='cursor-pointer text-slate-700 hover:text-500' onClick={()=>dispatch(deleteCartItem(id))}>
        <AiFillDelete/>
        </div>
        </div>

          <p className='text-slate-500m font-medium'>{category}</p>
